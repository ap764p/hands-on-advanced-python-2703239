...
import pprint #to load a specific print function
...
pprint.pp(weatherdata[0]) #0 provides first record
...
for d in weatherdata:
        key = d['date'][0:4] #allows getting date from json file, 4 characters specific for year
        if key in years:
            years[key] +=1
        else:
             years[key] =1
pprint.pp(years,width=5) #pretty print 